...
while True:
    if i == 4:
        ...
        time.sleep(3)
        i += 1
print(f"The closest suitable landing area has vertices ({points[i][0]},{points[i][1]}), ({points[i][2]},{points[i][3]}), and ({points[i][4]},{points[i][5]}) . The area is {area} km^2 and the centroid is {distance} km away from base.")